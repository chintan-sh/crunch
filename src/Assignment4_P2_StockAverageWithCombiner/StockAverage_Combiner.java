 ...
 * @author Chintan
 */
public class StockAverage_Combiner extends Reducer<Text, StockAverage_CompositeValueWritable, Text, StockAverage_CompositeValueWritable> {
    ...
    public void reduce(Text key, Iterable<StockAverage_CompositeValueWritable> values,  Context context) throws IOException, InterruptedException {
        ...
        for (StockAverage_CompositeValueWritable val : values) {
            sum += Float.parseFloat(val.getAverage());
            count = count + 1;
        }
